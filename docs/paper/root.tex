...
\abstract{Greenhouse tomato inspection requires mobile image acquisition across narrow aisles and terrain transitions, together with interpretable records of crop ripeness. This study presents a wheel-legged quadruped platform combining LiDAR-based navigation, an RGB-D camera for near-field terrain sensing, and panoramic acquisition. Equirectangular images are geometrically reprojected into perspective views, and the left and right views are processed offline using a tomato detector followed by a ripeness classifier. Five field visits between October and December 2025 provided qualitative demonstrations of row traversal, inter-row transition, and obstacle negotiation in a commercial greenhouse. A separate reproducibility example comprises 13 sequential panoramas and 68 saved detection observations across 26 frame-side positions. Archived development-validation records report detector precision of 0.697, recall of 0.694, mAP at an intersection-over-union threshold of 0.5 of 0.697, and mAP averaged over thresholds from 0.5 to 0.95 of 0.339; classifier top-1 accuracy was 0.810 on 42 validation crops. These validation data were used for model selection and do not constitute an independent field benchmark. The panorama example uses an assumed trajectory and row geometry, so its three-dimensional display organizes observations rather than verifying fruit locations or unique-fruit counts. The results support the feasibility of bilateral panoramic acquisition and offline ripeness summarization and identify the measurements needed for spatially accurate crop mapping. Code and panorama examples for offline ripeness detection and spatial observation mapping are available at \url{https://github.com/Alaurb/prms-system}; navigation implementation is outside the repository scope.}
\keyword{tomato ripeness; wheel-legged quadruped; panoramic imaging; offline recognition; observation mapping; reproducibility}
\begin{document}
...
Four visually defined categories are used: immature, green mature, discoloration, and mature (Figure~\ref{fig2_maturity_period}). Table~\ref{table_maturity} summarizes their image-based interpretation. Internal seed development is not observable in the supplied external images and is not independently established by these labels.
% AUTHOR CHECK: Verify the exact applicability and wording of GH/T 1193-2021 before restoring the original standard-based classification claim.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.62\linewidth]{fig2_ripeness.pdf}
	\caption{Four visually defined ripeness stages: (a) immature, (b) green mature, (c) discoloration, and (d) mature.}
	\label{fig2_maturity_period}
\end{figure}
\begin{table}[H]
\caption{Visible appearance used to describe the four ripeness categories. Boundary cases require a consistent manual annotation protocol.}
\centering
\begin{tabular}{p{2.5cm}p{7.0cm}}
\toprule
Category & Visible appearance\\
\midrule
Immature & Predominantly green fruit with an apparently underdeveloped appearance; size alone is not a reliable criterion.\\
\midrule
Green mature & Developed fruit that remains green or whitish green; separation from immature fruit can be ambiguous in images.\\
\midrule
Discoloration & Visible transition in surface colour, including yellow or pale-red regions.\\
\midrule
Mature & Predominantly mature external colour for the cultivar; redness alone does not generalize to all cultivars.\\
\bottomrule
\end{tabular}
...
The supplied processing implementation uses a two-stage pipeline: a tomato detector produces candidate crops, and a classifier assigns ripeness categories. The classifier includes the four ripeness classes and an additional category for non-target or unsuitable crops, which is excluded from ripeness outputs. The native class labels are normalized to the four ripeness names used in this manuscript.
...
\begin{figure}[H]
	...
	\caption{Observation-processing workflow and geometric inputs required for metric mapping. In the supplied panorama example, pose and row geometry are assumed. Count stacks represent observations, not physical fruit heights.}
	\label{fig6_mapping}
	\end{adjustwidth}
\end{figure}
...
The saved-prediction example contains 68 detections across 13 panoramas and 26 left/right observation positions: 28 immature, 31 green-mature, nine discoloration-stage, and zero mature predictions. Twenty observation positions contain at least one detection. These are model-output counts. Zero mature predictions does not establish absence of mature fruit, and this sample cannot demonstrate performance on all four classes.

The example output was regenerated by replaying saved predictions. Its trajectory is synthetic, its row geometry assumed, and its columns frame-side proxies. It therefore supports inspection of data organization and visualization, but not metric fruit localization or unique-fruit counting. The example is distinct from the complete set of historical field illustrations.
...
\section{Conclusions}
This study presents a wheel-legged quadruped inspection workflow combining panoramic acquisition with offline tomato detection, ripeness classification and observation visualization. Field sequences qualitatively demonstrate selected mobility scenarios. Archived development-validation records provide limited quantitative evidence for recognition, while a 13-panorama example offers a tractable starting point for reproducing observation processing.

The current evidence does not establish independent field recognition accuracy, real-time end-to-end operation, unique-fruit counting or metric three-dimensional fruit positions. Priority evaluation tasks are independent image annotation, reconciliation of model and training provenance, measured camera geometry, and recorded navigation and localization trials. These additions would permit direct testing of the workflow's spatial and operational performance.
...
\dataavailability{The PRMS system repository is available at \url{https://github.com/Alaurb/prms-system}. The repository is organized around offline panoramic ripeness detection and spatial observation mapping, with interfaces for externally supplied camera poses and registered range data. The prepared reproduction package contains 13 frames, model files, and saved outputs. The example does not include measured LiDAR poses or ROS bag recordings. Navigation code is outside the repository scope, and the package does not reproduce the field navigation trials.}
