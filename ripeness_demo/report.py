...
function vineHeight(plant){return Math.max(1.9,.82+plant.cubes.length*.38)}function fruitPosition(plant,cube,index){const base=.52+index*.36+(Number.isFinite(cube.z)?Math.min(1.5,Math.max(0,cube.z))*.12:0),side=plant.side==='left'?1:-1,angle=index*2.17+(plant.side==='left'?.5:1.1);return{x:plant.x+side*(.25+.10*(index%3))+Math.cos(angle)*.25,y:plant.y+Math.sin(angle)*.28,z:Math.min(vineHeight(plant)-.16,base)}}
